...
class WeeklyReminderWorker(
    ...
    override fun doWork(): Result {
        return try {
            ...
            // Fix: Use sync method instead of LiveData for Worker
            ...
            customerGroups.forEach { (name, txList) ->
                ...
                // Fix: Calculate balance (Credit - Payment)
                val totalDue = txList.sumOf { 
                    if (it.type == "CREDIT") it.amount else -it.amount 
                }

                if (!phone.isNullOrEmpty() && totalDue > 0) {
                    val message = "Dear $name, your total pending due at $shopName " +
                            "is ₹${String.format("%.2f", totalDue)}. " +
                            "Kindly clear it soon. Thank you!"
